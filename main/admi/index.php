<?php
  ...
  if(isset($_POST['key']))
  {
    $query = "INSERT INTO `invitekeys`(`invitekey`, `id`) VALUES (:key, :id)";
    $key = $db->prepare($query);
    $key = $key->execute(['key' => filter_var($_POST['key']), 'id' => $user['id']]);
    if($key)
    {
     $reason = 'Successfully generated key!';
    } else {
     $reason = 'Could not generate key. Try again later.'; 
    }
  }
?>
...
<div class="nav-icon" onselectstart="return false;" >
    ...
    <div class="container-main">
      ...
      <h2>Soulfull Admin Panel (W.I.P)</h3><br>
        <h3>Invite Key Generation</h3>

        <form method="POST">
          <input name="key" placeholder="Soulfull_1"><br>
         <input type="submit"> 
        </form>

        <br><button><a href="invitekeys">Click here to see all generated invite keys</a></button>
